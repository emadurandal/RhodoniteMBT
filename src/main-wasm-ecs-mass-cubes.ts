import "./style.css";
import wasmUrl from "../_build/wasm-gc/debug/build/emadurandal/rhodonite_examples/ecs-mass-cubes/wasm/main/main.wasm?url";
...
function updatePerfOverlay(fps: number, cpuMs: number): void {
	...
	el.textContent = `FPS ${fps.toFixed(1)}  ·  WASM CPU ${cpuMs.toFixed(2)} ms / frame (submit まで)`;
}

function createHostImports(renderer: Renderer): WebAssembly.Imports {
	return {
		rhodonite_ecs_mass_cubes_host: {
			...
			render_frame: (fps: number, cpuMs: number) => {
				...
				updatePerfOverlay(fps, cpuMs);
			},
		},
	};
}

async function instantiateWasm(renderer: Renderer): Promise<WasmExports> {
	...
	const { instance } = await WebAssembly.instantiate(bytes, createHostImports(renderer));
...
if (!navigator.gpu) {
	...
	window.addEventListener("load", () => {
		...
		void createRenderer(canvas)
			.then(async (renderer) => {
				const wasm = await instantiateWasm(renderer);
				wasm._start?.();
