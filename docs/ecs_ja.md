...
| `GpuWriteBytes` | native 用 upload slice（`byte_offset` + `Bytes`）。フルストア範囲では ECS GPU store の backing を借用できます。 |
...
- **`drain_gpu_write_bytes(component)`**: native 専用の drain で、`GpuWriteBytes`（`byte_offset` + `Bytes`）を返します。dirty span が native GPU store backing 全体を覆う場合は backing を借用し、部分 span は compact な所有 payload に fallback します。
...
- `GPUQueue::write_buffer`: ECS が返す native `GpuWriteBytes` payload 向け。フルストア範囲では generic な `ArrayView -> Bytes::from_array` fallback を通さずに upload できます。

実サンプル: [`ecs-scene-graph` の `render_frame`](../moon/rhodonite_examples/src/ecs-scene-graph/common/webgpu_renderer.mbt) は所有型 drain path を使います。高負荷サンプルの [`ecs-mass-cubes`](../moon/rhodonite_examples/src/ecs-mass-cubes/common/webgpu_renderer.mbt) は `spawn_transform_global_batch` を使い、JS では `write_global_transforms_dense_grid_wave_views` / `drain_gpu_write_views` / `queue.write_buffer_from_array_view`、native では `write_global_transforms_dense_grid_wave_bytes` / `drain_gpu_write_bytes` / `queue.write_buffer` を使います。
