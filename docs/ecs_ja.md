...
`Schedule` は固定 lifecycle を持たず、外部から与えられる `SystemPhase` token と phase order に従って実行されます。`Schedule::run` / `Schedule::run_phase` 中の `World` mutation API は `System` の access 宣言で guard されます。
