...
- `reserve_batch_capacity(components, additional_rows)` は archetype signature に対する row capacity を事前確保します。大量 spawn する数が事前に分かっている場合は、spawn/write の hot path から capacity 拡張を外すために先に呼びます。
...
この API での `Schedule` は、global な frame timeline を所有する object ではなく、phase filtering を持つ system list / runner です。phase の意味と順序は caller / runtime 側の境界にあります。
