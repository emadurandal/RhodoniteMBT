...
| `Engine` | `GPUContext`、scene collection、main scene index、time state、frame phase order を持ち、`tick(app)` で標準 phase を順に実行する。`insert_phase_before` / `insert_phase_after` で独自 phase を挿入できるが、app 初期化または初回実行後の phase order 変更は error にする。 |

`Engine::initialize_app(app)`、`Engine::tick(app)`、`Engine::run_phase(app, phase, frame)` は app の登録 handler が `phase_startup()`、`phase_shutdown()`、または engine の frame phase order に含まれる phase だけを使っているか検証します。未登録 phase を `App::on_phase` に渡した場合は、初回実行時に error になります。独自 phase を使う場合は、app を走らせる前に `Engine::insert_phase_before` / `Engine::insert_phase_after` で engine の phase order に追加します。
