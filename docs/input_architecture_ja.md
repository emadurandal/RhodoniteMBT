...
MoonBit 実装の `ecs-mass-cubes` と `ecs-scene-graph` は `rhodonite_core/ecs` の `OrbitCameraController` component を使う。これは yaw / pitch / pan / dolly / distance、sensitivity、clamp、reset request を保持する CPU component で、`Engine::add_orbit_camera_controller_input_handler` が `rhodonite/input` phase で `Engine::input()` から mouse drag delta を読んで component を更新する。

camera matrix の生成側は DOM や SDL3 を見ず、camera entity の `Transform3D` / `GlobalTransform` / `CameraLens` を読む。標準経路では `Scene::add_orbit_camera_systems` が `orbit_camera_transform_system` と `orbit_camera_blob_sync_system` を登録し、`OrbitCameraController + CameraHomeTransform` から `Transform3D` を更新してから `GlobalTransform + CameraLens + OrbitCameraController.dolly` から builtin `Camera` packed blob を更新する。reset は入力 binding には固定せず、ユーザーまたは sample が `request_orbit_camera_reset` を呼ぶ one-shot request として扱う。`TransformPropagation` も `Scene::add_transform_propagation_system` で scene schedule に置く。
