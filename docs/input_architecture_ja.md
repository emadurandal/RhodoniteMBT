...
TypeScript 実装の `ts-ecs-mass-cubes` と、WASM / WASM-GC 実装の `ecs-mass-cubes` host は [`src/orbit-camera-controller.ts`](../src/orbit-camera-controller.ts) の同等 controller を使う。`ts-ecs-mass-cubes` は `Engine.input` を使い、WASM host は host 側で所有する `InputState` の集約済み mouse frame 値を WASM import として渡す。どちらも [`installBrowserInputState`](../src/app-runtime.ts) で browser pointer event を同じ入力モデルへ正規化する。`ts-ecs-mass-cubes` は builtin `Camera` component を camera entity に持たせ、render 直前に controller から view/proj を再生成して TS ECS wrapper の `World.setCameraMatrices` 経由で packed camera blob を更新し、`drainAndUploadCameraWrites` で GPU storage buffer へ反映する。WASM / WASM-GC 版も WASM 側 `World` に camera entity と `OrbitCameraController` component を持たせ、WASM 側で controller 更新、view/proj 生成、`World::set_camera_matrices`、packed camera blob row 読み出しまで行う。host は WebGPU の camera storage buffer へ 72 u32 words を upload し、render pass を発行する。描画定数、camera binding 方式ごとの pipeline / buffer 作成、render pass は [`src/ecs-mass-cubes-renderer.ts`](../src/ecs-mass-cubes-renderer.ts) に寄せ、TS ECS backend と WASM backend の transform 更新経路は entry ごとに残す。
