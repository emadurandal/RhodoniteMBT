...
- MoonBit browser platform: `emadurandal/rhodonite/app/browser` の `run_platform(config, app, options)`、`stop_platform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`start_platform(engine, canvas, options)`、`gpu_context_from_html_canvas(canvas)`、`start_browser_engine_canvas_loop(engine, canvas)`、`sync_browser_engine_surface(engine, canvas)`、`install_browser_input(engine, canvas)`。`run_platform` は canvas 単位で既存 platform を置換する。
- Browser TypeScript platform: [`src/app-runtime.ts`](../src/app-runtime.ts) の `runBrowserWebGpuCanvasDemo(...)`、`runPlatform(config, app, options)`、`stopPlatform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`startPlatform(engine, options)`、`installBrowserInputCallbacks(...)`、`installBrowserInput(engine)`。`runPlatform` は canvas 単位で既存 platform を置換する。
- Native SDL3 platform: `emadurandal/rhodonite_app_sdl3/sdl3` の `run_platform(config, app, options)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`init_sdl_metal_webgpu_native(...)`、`run_sdl_metal_webgpu_input_render_loop(...)`。
