...
- MoonBit browser platform: `emadurandal/rhodonite/app/browser` の `run_platform(config, app, options)`、`stop_platform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`FrameMode`、`start_platform(engine, canvas, options)`、`gpu_context_from_html_canvas(canvas)`、`start_browser_engine_canvas_loop(engine, canvas)`、`sync_browser_engine_surface(engine, canvas)`、`install_browser_input(engine, canvas)`。`PlatformOptions::interactive()` は継続描画と入力を有効化し、`PlatformOptions::render_loop()` は入力なしの継続描画、`PlatformOptions::single_frame()` は入力なしで1フレームだけ描画する。`run_platform` は canvas 単位で既存 platform を置換する。
- Browser TypeScript platform: [`src/app-runtime.ts`](../src/app-runtime.ts) の `runBrowserWebGpuCanvasDemo(...)`、`runPlatform(config, app, options)`、`stopPlatform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`startPlatform(engine, options)`、`installBrowserInputCallbacks(...)`、`installBrowserInput(engine)`。`PlatformOptions.frameMode` は `"loop"` または `"single-frame"` を選び、`PlatformOptions.renderLoop()` は入力なしの継続描画、`PlatformOptions.singleFrame()` は入力なしで1フレームだけ描画する。`runPlatform` は canvas 単位で既存 platform を置換する。
- Native SDL3 platform: `emadurandal/rhodonite_app_sdl3/sdl3` の `run_platform(config, app, options)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`FrameMode`、`init_sdl_metal_webgpu_native(...)`、`run_sdl_metal_webgpu_input_render_loop(...)`。SDL3 でも `PlatformOptions::interactive()` は継続描画と入力を有効化し、`PlatformOptions::render_loop()` は window/surface event を処理しつつ engine input queue への転送を止め、`PlatformOptions::single_frame()` は初期化後に1フレームだけ描画して終了する。
