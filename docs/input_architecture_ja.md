...
`emadurandal/rhodonite/app` の `Engine` は `InputState` を所有し、`Engine::input()` で公開する。`Engine::run_render_frame(delta_seconds)` はフレーム冒頭で `input.begin_frame()` を呼び、前フレームから enqueue されたイベントを現在フレームの状態へ確定する。
