...
rhodonite/frame_begin:
  rhodonite/input
rhodonite/fixed_step 0..max_fixed_steps_per_frame:
...
```text
emadurandal/rhodonite/app/browser install_browser_input / start_platform / run_platform
または src/app-runtime.ts installBrowserInputCallbacks
  -> engine.input().enqueue_event(...)
app/browser start_browser_engine_loop
または startPlatform / runPlatform
  -> engine.run_frame(delta_seconds)
     -> input.begin_frame()
     -> rhodonite/input
     -> rhodonite/fixed_update / rhodonite/fixed_post_update 0..max_fixed_steps_per_frame
     -> rhodonite/update
```
...
この順序により、DOM の非同期イベントと SDL の polling はどちらも同じ queue に合流し、app 側は `key_down`, `key_pressed`, `mouse_down`, `pointer_delta_x` などの状態 API だけを読む。background tab 復帰や OS suspend 復帰で大きな `delta_seconds` が入った場合も、固定ステップは既定で 5 回に打ち切り、超過 backlog を破棄する。
...
- MoonBit browser platform: `emadurandal/rhodonite/app/browser` の `run_platform(config, app, options)`、`stop_platform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`FrameMode`、`start_platform(engine, canvas, options)`、`gpu_context_from_html_canvas(canvas)`、`start_browser_engine_canvas_loop(engine, canvas)`、`start_browser_engine_on_demand_canvas_loop(engine, canvas)`、`sync_browser_engine_surface(engine, canvas)`、`install_browser_input(engine, canvas)`。`PlatformOptions::interactive()` は継続描画と入力を有効化し、`PlatformOptions::render_loop()` は入力なしの継続描画、`PlatformOptions::interactive_on_demand()` は入力ありの on-demand 描画、`PlatformOptions::render_on_demand()` は入力なしの on-demand 描画、`PlatformOptions::single_frame()` は入力なしで1フレームだけ描画する。`run_platform` は canvas 単位で既存 platform を置換する。
- Browser TypeScript platform: [`src/app-runtime.ts`](../src/app-runtime.ts) の `runBrowserWebGpuCanvasDemo(...)`、`runPlatform(config, app, options)`、`stopPlatform(canvas)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`startPlatform(engine, options)`、`installBrowserInputCallbacks(...)`、`installBrowserInput(engine)`。`PlatformOptions.frameMode` は `"loop"`、`"on-demand"`、`"single-frame"` を選び、`PlatformOptions.renderLoop()` は入力なしの継続描画、`PlatformOptions.interactiveOnDemand()` は入力ありの on-demand 描画、`PlatformOptions.renderOnDemand()` は入力なしの on-demand 描画、`PlatformOptions.singleFrame()` は入力なしで1フレームだけ描画する。`runPlatform` は canvas 単位で既存 platform を置換する。
- Native SDL3 platform: `emadurandal/rhodonite_app_sdl3/sdl3` の `run_platform(config, app, options)`、`PlatformConfig`、`PlatformApp`、`PlatformOptions`、`FrameMode`、`init_sdl_metal_webgpu_native(...)`、`run_sdl_metal_webgpu_input_render_loop(...)`、`run_sdl_metal_webgpu_input_on_demand_render_loop(...)`。SDL3 でも `PlatformOptions::interactive()` は継続描画と入力を有効化し、`PlatformOptions::render_loop()` は window/surface event を処理しつつ engine input queue への転送を止め、`PlatformOptions::interactive_on_demand()` は入力ありの on-demand 描画、`PlatformOptions::render_on_demand()` は入力なしの on-demand 描画、`PlatformOptions::single_frame()` は初期化後に1フレームだけ描画して終了する。
