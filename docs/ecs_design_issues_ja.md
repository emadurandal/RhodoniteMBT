...
これにより、`QueryRow::write_view` で GPU-visible row を変更したのに dirty を立て忘れる問題は縮小しています。`update_global_transforms_from_transforms` も `GlobalTransform` の `write_view` を取得することで dirty を記録します。
...
- 現行実装では、まず最小の `CommandBuffer` を追加済み。query 中に積んだ `destroy`、`add/remove component`、`set component bytes`、`clear GPU component` を query 後に適用できる。
