...
- native では `drain_gpu_write_bytes` により、フルストア upload span を `Bytes` として借用し、`GPUQueue::write_buffer` に直接渡せる。
...
- native backend の汎用 `write_buffer_from_array_view` は現状 `Bytes` 経由で fallback copy する。ECS のフルストア hot path は `GpuWriteBytes` で回避済みだが、任意の `ArrayView[Byte]` upload まで JS と同等にするには追加の queue binding が必要。
...
- ECS-to-GPU upload の hot path では JS は `drain_gpu_write_views` と `GPUQueue::write_buffer_from_array_view`、native は `drain_gpu_write_bytes` と `GPUQueue::write_buffer` を使い、owned bytes が必要な path だけ `drain_gpu_writes` を使う。
...
- native backend の汎用 `write_buffer_from_array_view` を lower-level queue binding に寄せ、任意の `ArrayView[Byte]` から upload までの fallback copy をなくす。ECS のフルストア upload は `GpuWriteBytes` で先に回避済み。
