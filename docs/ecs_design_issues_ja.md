...
- `GpuVisible` component の dirty は `QueryRow::write_view` 取得時に自動記録されるが、実際に bytes が変わったかまでは見ないため、最小 dirty ではない。bulk path では dirty range を直接積め、即時 upload では `drain_gpu_write_views`、native では `drain_gpu_write_bytes` で payload copy なしの borrowed span を返せるが、row view 経由の最小 dirty 問題は残る。
...
- `drain_gpu_write_views` により、renderer がすぐ upload する path では `GpuComponentStore` の backing bytes を借用 view として渡せる。
- native では `drain_gpu_write_bytes` により、フル範囲・部分範囲の upload span を `Bytes` + source offset/length として借用し、`GPUQueue::write_buffer_from_bytes_range` に直接渡せる。
...
`QueryRow::read_view` / `QueryRow::write_view` は `ComponentColumn::bytes` の row 範囲を view として返します。`GpuVisible` の場合も `GpuComponentStore.bytes` の entity slot を直接 view にします。CPU-only のホットループでは `Query::for_each_archetype` が列 view を返し、マッチした archetype ごとに component column index / stride を cache します。
...
現在の `update_global_transforms_from_transforms` はこの利点を活かして、`Transform3D` の SoA row を読み、`GlobalTransform` の GPU row へ直接書いています。`update_transform3d_positions` も正式な bulk API として追加され、JS では公開 archetype-column API、非 JS では direct archetype sweep で position だけを更新します。

大量生成向けには `World::spawn_transform_global_batch` が追加されています。これは builtin `[Transform3D, GlobalTransform]` archetype に entity を直接 append し、callback に CPU row / GPU row の mutable view を渡します。`Transform3D::write_trs_to_component_mut_view` や `global_transform_write_identity_row` と組み合わせることで、component 追加時の temporary bytes と archetype migration を避けられます。
...
`GpuComponentStore::set_bytes` や `mut_entity_row` は `mark_dirty` します。現行実装では、`QueryRow::write_view` が `GpuVisible` の `MutArrayView[Byte]` を返す時点でも dirty を記録します。

これにより、`QueryRow::write_view` で GPU-visible row を変更したのに dirty を立て忘れる問題は縮小しています。さらに `update_global_transforms_from_transforms` などの bulk path は、連続した `EntityId.index` 群を dirty range として直接記録し、`drain_gpu_writes` が個別 dirty index と range を統合して upload span を作ります。

即時 upload する renderer では `drain_gpu_write_views` を使うと、`GpuWrite` の owned payload ではなく `GpuWriteView` の `ArrayView[Byte]` として dirty span を取得できます。JS backend では `GPUQueue::write_buffer_from_array_view` がこの view を `Uint8Array.subarray` として渡すため、ECS drain から WebGPU upload までの追加 payload copy を避けられます。native の高負荷 path では `drain_gpu_write_bytes` が `GpuWriteBytes` を返し、フル範囲・部分範囲とも `FixedArray[Byte]` backing を `Bytes` として借用し、source offset/length を `GPUQueue::write_buffer_from_bytes_range` へ渡せます。

残る注意点:

- 自動 dirty は「実際に bytes が変わった row」ではなく、「GpuVisible の mutable view を要求した row」に立つ。
- `write_view` を取得して実際には書かなかった場合も upload 対象になる。
- 読み取りだけなら `read_view` を使う規律が必要。
...
- `GpuWriteBytes` も GPU component store の backing bytes を借用する場合があるため、同じ store を resize / mutate する前に upload まで終える必要がある。
- native backend の汎用 `write_buffer_from_array_view` は現状 `Bytes` 経由で fallback copy する。ECS の hot path は `GpuWriteBytes` + `write_buffer_from_bytes_range` でフル範囲・部分範囲とも回避済みだが、任意の `ArrayView[Byte]` upload まで JS と同等にするには source offset を持つ追加 API が必要。
...
現行実装では `ComponentColumn` が `row_capacity` を持ちます。`push_empty_row` は capacity を幾何級数的に伸ばし、削除時に backing bytes を毎回 shrink しません。`QueryArchetype::read_column` / `write_column` は `entities.length() * stride` の論理長だけを返すため、未使用 capacity は公開 view に出ません。
...
`find_archetype` は全 archetype を線形探索し、`component_array_contains_all` や初回 `column_index` も線形探索です。小規模なら問題ありませんが、component 種類や archetype 数が増えると query overhead が増えます。現行実装では `Query::for_each_archetype` が、マッチした archetype ごとに required component の column index / stride を cache し、`read_column` / `write_column` 側の繰り返し探索は避けています。
...
- query plan cache。
- より長寿命な query plan cache / archetype match cache。
...
- `GpuVisible` view の dirty 漏れを防ぐ。現行実装では `QueryRow::write_view` が `GpuVisible` component の mutable view を返す時点で dirty を立てる。
...
- System は read/write component set を宣言する。`reads` は読み取り専用、`writes` は読み書き可能 access を意味する。現行実装では、schedule 実行中の `component_bytes` / `Query` required component / `QueryRow::read_view` は `reads ∪ writes`、`QueryRow::write_view` / `set_component_bytes` / GPU write / dirty / drain は `writes` に含まれる component だけを許す。
...
- ECS-to-GPU upload の hot path では JS は `drain_gpu_write_views` と `GPUQueue::write_buffer_from_array_view`、native は `drain_gpu_write_bytes` と `GPUQueue::write_buffer_from_bytes_range` を使い、owned bytes が必要な path だけ `drain_gpu_writes` を使う。
- 大量の builtin transform entity 作成では `spawn_transform_global_batch` と direct row writer を使い、通常の component 追加 path は汎用性が必要な場合に残す。
...
- query plan cache と archetype index cache を追加する。現行の per-matched-archetype column cache は短期的な改善として入っている。
- native backend の汎用 `write_buffer_from_array_view` を lower-level queue binding に寄せ、任意の `ArrayView[Byte]` から upload までの fallback copy をなくす。ECS upload は `GpuWriteBytes` + `write_buffer_from_bytes_range` で先に回避済み。
