...
- `MutArrayView[Byte]` が内部 storage の可変参照を直接公開しており、借用範囲、排他性、構造変更禁止を API として表現できていない。
- `for_each_entity_with_components` の callback 中に `World` の構造変更 API を呼べてしまい、走査中の archetype / row / location の整合性が壊れうる。
- `GpuVisible` component を `MutArrayView[Byte]` 経由で変更しても dirty が自動記録されず、呼び忘れで GPU upload が欠落する。
- `World` 全体が `Array` ベースの可変所有物で、query borrow や read/write set によるスケジューリング境界がまだない。
...
`for_each_entity_with_components` は `self.archetypes` と `arch.entities` を直接走査します。一方で callback には `World` を閉じ込めて呼べるため、callback 内で次のような操作が可能です。
...
このため、System 化する場合は query callback 中の構造変更を禁止し、`CommandBuffer` に遅延する設計が必要です。既存の [`ecs_system_design_ja.md`](ecs_system_design_ja.md) の `CommandBuffer` 方針はこの問題に対応しています。
...
- Query 実行中は構造変更しない。
- System ごとに read component / write component を宣言する。
- 同じ component への write が重なる System は同時実行しない。
...
2. System に `reads` / `writes` component set を持たせる。
3. Query callback 中の構造変更を禁止する。
4. `for_each_entity_with_components` を低レベル API と位置付け、System 用の wrapper を追加する。
5. `World` 内部に query borrow / execution guard を入れ、構造変更 API が query 中に呼ばれたら失敗または abort する。
...
- component 登録は world 初期化フェーズに限定する。
- Schedule 実行開始後の登録は禁止または明示的な setup API に隔離する。
...
`required` は `Array[ComponentTypeId]` のため、重複や未ソートを許します。未ソートは payload 順序として意味がありますが、重複 component が渡された場合の意味は曖昧です。

推奨:

- duplicate required を検出して abort / false にする。
...
- `for_each_entity_with_components` を「低レベル、単一スレッド、構造変更禁止」の API と明記する。
- Query callback 中の `add/remove/destroy/register` を禁止事項として docs に追記する。
- `GpuVisible` view を書いたら `mark_gpu_component_dirty` が必要なことを、API 名や helper で目立たせる。
- `required` の重複検査を追加する。

### 中期

- `Schedule` / `System` / `CommandBuffer` を導入する。
- System は read/write component set を宣言する。
- System 用 Query wrapper を作り、通常は typed wrapper または role 付き payload を使う。
- `World` に query 実行中 guard を追加し、構造変更 API を callback 中に呼んだ場合は失敗させる。

### 長期

- read/write set に基づく System conflict 検査を追加する。
