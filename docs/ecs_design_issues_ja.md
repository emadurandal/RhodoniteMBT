...
- System は read/write component set を宣言する。`reads` は読み取り専用、`writes` は読み書き可能 access を意味する。現行実装では、schedule 実行中の `component_bytes` / `gpu_component_bytes` / query required は `reads ∪ writes`、`add_component_bytes` / `remove_component` / GPU write / dirty / drain は `writes` に含まれる component だけを許す。
...
- 現行実装の注意点として、`add_component_bytes` / `remove_component` は archetype 移動を起こす構造変更でもあるが、現在は対象 component の `writes` 宣言で許可している。より厳密には、将来 `structural_write` も要求するか、component 追加/削除専用の権限を分ける設計が必要。
