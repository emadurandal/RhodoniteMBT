...
- `add_cpu_component_bytes` / `add_gpu_component` / 互換用 `add_component_bytes`
- `remove_component`
- `destroy_entity`
- `create_entity`
...
ただし、これは「禁止」の仕組みであり、System から構造変更を安全に要求する仕組みではありませんでした。現行実装では `CommandBuffer` を追加済みで、`destroy`、CPU component 追加、GPU-visible component 所有追加、`remove component`、`set/clear GPU component` を query / system 後に適用できます。`CreateEntity` command はまだ含めず、query 走査外では `World::create_entity` を直接使う方針です。
...
`GpuVisible` は archetype signature には含まれますが、archetype column stride は `0` です。このため、「component を持つかどうか」と「payload はどこにあるか」が分離しています。現行実装では、所有 marker の追加は `add_gpu_component`、payload の更新は `set_gpu_component_bytes` に分け、CPU payload を伴う `add_cpu_component_bytes` と API 名でも区別しています。互換用の `add_component_bytes` は残していますが、新しいコードでは明示 API を使う方針です。
...
- `add_cpu_component_bytes` / `add_gpu_component` / `add_component_bytes` / `remove_component` は archetype 移動を起こす構造変更なので、対象 component の `writes` に加えて `structural_write` も要求する。既存 CPU component の bytes 更新は `set_component_bytes`、GPU-visible payload 更新は `set_gpu_component_bytes` に分離済みで、これは archetype 構造を変えないため `writes` のみでよい。
