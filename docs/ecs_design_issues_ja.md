...
- `for_each_entity_with_components` を「低レベル、単一スレッド、構造変更禁止、内部用途寄り」の API と明記する。現行実装ではコメントと `query_depth` guard で対応済み。さらに schedule 実行中に直接使う場合は、全 required component を `writes` に宣言する必要がある。
...
- System は read/write component set を宣言する。`reads` は読み取り専用、`writes` は読み書き可能 access を意味する。現行実装では、schedule 実行中の `component_bytes` / `gpu_component_bytes` / `QueryRow::read_view` は `reads ∪ writes`、`QueryRow::write_view` / `set_component_bytes` / GPU write / dirty / drain は `writes` に含まれる component だけを許す。低レベルの `World::for_each_entity_with_components` は全 payload を mutable view として渡すため、全 required component に `writes` を要求する。
...
- `add_component_bytes` / `remove_component` は archetype 移動を起こす構造変更なので、対象 component の `writes` に加えて `structural_write` も要求する。既存 component の bytes 更新は `set_component_bytes` に分離済みで、これは archetype 構造を変えないため `writes` のみでよい。
