...
- `GpuVisible` component の dirty は `QueryRow::write_view` 取得時に自動記録されるが、実際に bytes が変わったかまでは見ないため、最小 dirty ではない。bulk path では dirty range を直接積めるようになったが、row view 経由の最小 dirty 問題は残る。
...
### 5. CPU column の backing capacity と論理行数は分離済み
...
- ECS microbench（`pnpm run bench:ecs:*`）で `QueryRow`、`for_each_archetype`、builtin bulk path、`drain_gpu_writes` を継続測定する。
