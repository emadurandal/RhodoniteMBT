...
- `for_each_entity_with_components` の callback 中の構造変更は `query_depth` guard で実行時に禁止しているが、CommandBuffer などの遅延構造変更 API はまだない。
...
### 3. Query 中の構造変更は guard 済みだが、遅延変更 API は未整備
...
ただし、これは「禁止」の仕組みであり、System から構造変更を安全に要求する仕組みではありません。System 化する場合は、引き続き `CommandBuffer` に遅延する設計が必要です。既存の [`ecs_system_design_ja.md`](ecs_system_design_ja.md) の `CommandBuffer` 方針はこの次段階に対応しています。
...
5. `query_depth` guard では表現できない read/write 排他性を System 側の宣言で検査する。
