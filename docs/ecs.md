...
The browser-only [`ts-ecs-mass-cubes`](../demos/ts-ecs-mass-cubes.html) demo uses the TypeScript ECS wrapper for the same dense-range callback path and submits borrowed write views with the native browser WebGPU API. The [`wasm-ecs-mass-cubes`](../demos/wasm-ecs-mass-cubes.html) and [`wasm-gc-ecs-mass-cubes`](../demos/wasm-gc-ecs-mass-cubes.html) demos build release MoonBit `wasm` / `wasm-gc` entrypoints for the dense ECS update and call a TypeScript host bridge for browser WebGPU work. The host bridge owns WebGPU objects and `GPUQueue.writeBuffer` uploads instead of passing MoonBit `ArrayView[Byte]` / `Bytes` directly across the WebAssembly host boundary.
