...
`Schedule` does not own a fixed update/render lifecycle. Callers supply `SystemPhase` tokens and either run one phase with `Schedule::run_phase` or pass an explicit phase order to `Schedule::run`. `World` mutation APIs are guarded by `System` access declarations during schedule execution:
