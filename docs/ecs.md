...
- `reserve_batch_capacity(components, additional_rows)` pre-allocates row capacity for an archetype signature. Use it before known large batch spawns to keep capacity growth out of the spawn/write hot path.
...
| `create_entity`, `destroy_entity`, `add_component*`, `remove_component`, `reserve_batch_capacity`, `spawn_batch` | `structural_write` plus relevant component `writes` |
...
- `World.reserveBatchCapacity`, `spawnBatch`
